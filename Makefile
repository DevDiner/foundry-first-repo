#Makerfile is to automate scripts by customizing your own command

#.PHONY: all clean install: This line declares all, clean, and install as phony targets. Make will always execute these targets when called, even if files with those names exist.
.PHONY: all test clean deploy fund help install snapshot format anvil zktest

#Makefile is attempting to include a .env file, which often contains environment variables or configurations. 
#The dash before include makes it so that if .env does not exist, the Makefile will continue executing without raising an error.
-include .env

#To run command, type "make build" , then it will run forge build. The ";" means to run the command in the same line.
build:; forge build

#in Makefile, env variables need to be enclosed by "()".
#can run this code with "make deploy-sepolia"
deploy-sepolia: 		
...
# run deploy ARGS="--network sepolia"
deploy:
